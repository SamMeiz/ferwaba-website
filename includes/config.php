...
// Ensure a standings row exists for a team/division/gender/group
function ensure_standings_row(mysqli $db, int $teamId, string $division, string $gender, ?string $team_group = null): void
{
    ...
    if ($row = $res->fetch_assoc()) {
        ...
        $upd->bind_param('siss', $team_group, $teamId, $division, $gender);
        $upd->execute();
    } else {
        $ins = $db->prepare("INSERT INTO standings(team_id, division, gender, team_group) VALUES(?,?,?,?)");
        $ins->bind_param('isss', $teamId, $division, $gender, $team_group);
        $ins->execute();
    }
}
...
function recalc_standings_for_game_change(mysqli $db, int $gameId): void
{
    ...
    $teamsRes = $db->query("SELECT id, team_group FROM teams WHERE division='$division' AND gender='$gender'");
    while ($t = $teamsRes->fetch_assoc()) {
        ensure_standings_row($db, (int) $t['id'], $game['division'], $game['gender'], $t['team_group']);
    }
