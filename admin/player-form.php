...
try {
  ...
  if ($editing) {
    $stmt = $db->prepare("SELECT team_id, name, position, height, nationality, jersey_number, photo FROM players WHERE id=? LIMIT 1");
    $stmt->execute([$id]);
    $row = $stmt->fetch();
    if ($row) {
      $team_id = $row['team_id'];
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
      if ($editing) {
        $stmt = $db->prepare("UPDATE players SET team_id=?, name=?, position=?, height=?, nationality=?, jersey_number=?, photo=? WHERE id=? LIMIT 1");
        $stmt->execute([$team_id, $name, $position, $height, $nationality, $jersey_number, $uploadFileName, $id]);
        audit_log($db, 'Edit Player', "Updated player: $name (ID: $id)");
      } else {
        $stmt = $db->prepare("INSERT INTO players(team_id, name, position, height, nationality, jersey_number, photo) VALUES(?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$team_id, $name, $position, $height, $nationality, $jersey_number, $uploadFileName]);
        $newId = $db->lastInsertId();
        audit_log($db, 'Add Player', "Created player: $name (ID: $newId)");
      }
      redirect('players.php');
    }
  }
} catch (PDOException $e) {
  error_log("Player Form Error: " . $e->getMessage());
      ...
      <div class="card-body">
        ...
        <form method="post" enctype="multipart/form-data">
          <div class="grid col-2" style="margin-bottom:8px">
            ...
            <div>
              <label>Jersey #</label>
              <input type="number" name="jersey_number" value="<?php echo (int) $jersey_number; ?>"
          ...
          <div class="grid col-2" style="margin-bottom:8px">
            ...
            <div>
              ...
              <input type="text" name="position" value="<?php echo sanitize($position); ?>"
                style="width:100%;padding:8px;border:1px solid #e5e7eb;border-radius:8px">
            </div>
          </div>
          <div class="grid col-2" style="margin-bottom:8px">
            <div>
              <label>Height</label>
              <input type="text" name="height" value="<?php echo sanitize($height); ?>" placeholder="e.g., 6'5\" or
                195cm" style="width:100%;padding:8px;border:1px solid #e5e7eb;border-radius:8px">
            </div>
            <div>
