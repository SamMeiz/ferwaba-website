...
try {
  if ($editing) {
    $stmt = $db->prepare("SELECT name, description, category, price, image, is_active, team_id, gender FROM shop_items WHERE id=? LIMIT 1");
    $stmt->execute([$id]);
    $row = $stmt->fetch();
    if ($row) {
      $name = $row['name'];
    ...
    if (!$error) {
      if ($editing) {
        $stmt = $db->prepare("UPDATE shop_items 
                    ...
                    WHERE id=? LIMIT 1");
        $stmt->execute([$name, $description, $category, $price, $uploadFileName, $is_active, $team_id, $gender, $id]);
        audit_log($db, 'Edit Shop Item', "Updated shop item: $name (ID: $id)");
      } else {
        $stmt = $db->prepare("INSERT INTO shop_items(name, description, category, price, image, is_active, team_id, gender) 
                    VALUES(?,?,?,?,?,?,?,?)");
        $stmt->execute([$name, $description, $category, $price, $uploadFileName, $is_active, $team_id, $gender]);
        $newId = $db->lastInsertId();
        audit_log($db, 'Add Shop Item', "Created shop item: $name (ID: $newId)");
      }
      redirect('shop.php');
    }
  }
} catch (PDOException $e) {
  error_log("Shop Form Error: " . $e->getMessage());
      ...
      <form method="post" enctype="multipart/form-data">
        ...
        <div class="grid col-2">
          ...
          <div class="form-group">
            ...
            <input type="number" step="0.01" id="price" name="price" value="<?php echo sanitize((string) $price); ?>"
              class="form-control">
          </div>
        </div>
        ...
        <div class="grid col-2">
          ...
          <div class="form-group">
            ...
            <select id="team_id" name="team_id" class="form-control">
              ...
              <?php
              $teams_list = $db->query("SELECT id, name FROM teams ORDER BY name ASC")->fetchAll();
        ...
        <div class="form-group">
          ...
          <input type="file" id="image" name="image" accept="image/jpeg,image/png,image/gif,image/webp"
            class="form-control">
          <?php if ($image): ?>
            <div class="muted">Current: <img src="uploads/<?php echo sanitize($image); ?>"
        ...
        <div class="form-group">
          ...
          <textarea id="description" name="description" rows="5"
            class="form-control"><?php echo sanitize($description); ?></textarea>
        </div>
