...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unlock_ip_address'])) {
  $token = $_POST['csrf_token'] ?? '';
  if (!verify_csrf_token($token)) {
    $security_message = 'Invalid request. Please try again.';
    $security_message_type = 'message-error';
  } elseif (current_admin_role() !== 'SuperAdmin') {
    $security_message = 'Only SuperAdmin can unlock IPs.';
    $security_message_type = 'message-error';
  } else {
    $unlockIp = $_POST['unlock_ip_address'];
    $db->prepare("DELETE FROM ip_bans WHERE ip_address=?")->execute([$unlockIp]);
    $db->prepare("DELETE FROM login_attempts WHERE ip_address=?")->execute([$unlockIp]);
    audit_log($db, 'IP Unlocked', "IP address $unlockIp was manually unlocked.");
    $security_message = "IP address $unlockIp unlocked successfully.";
    $security_message_type = 'message-success';
  }
}
...
try {
  ...
  $top_ip_attempts = $db->query("
    SELECT l.ip_address, COUNT(*) AS attempts, SUM(l.is_successful=0) AS failed_attempts, MAX(l.attempt_time) AS last_seen,
    (SELECT COUNT(*) FROM ip_bans b WHERE b.ip_address = l.ip_address) as is_banned
    FROM login_attempts l
    WHERE l.attempt_time > DATE_SUB(NOW(), INTERVAL 24 HOUR) 
...
<div class="tabs-navigation">
  ...
  <button class="tab-btn" onclick="switchTab('security')">
    <i class="fas fa-shield-alt"></i> Security Hub
  </button>
</div>
...
<div id="tab-security" class="tab-pane">
  ...
  <div class="dash-layout" style="margin-top: 0;">
    <!-- Security Main Column -->
    <div class="main-col">
      <div class="activity-card">
        <div class="activity-header">
          <h3><i class="fas fa-shield-alt"></i> Security Monitoring (Failed Logins)</h3>
        </div>
        <div class="activity-list">
                ...
                <div class="item-details">
                  ...
                  <?php if ($admin): ?>
                    <p style="font-size: 12px; color: var(--gray-600); margin-top: 4px;">
                      <i class="fas fa-user"></i> <?php echo sanitize($admin['full_name']); ?> (<?php echo sanitize($email); ?>)
                    </p>
                  <?php endif; ?>
                </div>
                <?php if ($admin && current_admin_role() === 'SuperAdmin'): ?>
                  <form method="post" style="margin-left: 12px;">
                    ...
                    <button type="submit"
                      ...
                      <?php echo $admin['is_active'] ? '<i class="fas fa-lock"></i> Lock' : '<i class="fas fa-unlock"></i> Unlock'; ?>
                    </button>
                  </form>
                <?php endif; ?>
              </div>
        ...
        <div class="activity-list">
          <?php if ($top_ip_attempts && count($top_ip_attempts) > 0): ?>
            <?php foreach ($top_ip_attempts as $ip): ?>
              <div class="activity-item" style="padding: 14px 24px; <?php echo !empty($ip['is_banned']) ? 'background: #fff5f5; border-left: 3px solid var(--danger);' : ''; ?>">
                <div class="item-details">
                  <h4 style="font-size: 14px; <?php echo !empty($ip['is_banned']) ? 'color: var(--danger);' : ''; ?>">
                    <?php echo sanitize($ip['ip_address']); ?>
                    <?php if (!empty($ip['is_banned'])) echo ' <i class="fas fa-ban" title="Banned"></i>'; ?>
                  </h4>
                  <p style="font-size: 11px;">
                    <strong style="color: var(--danger);"><?php echo (int) $ip['failed_attempts']; ?> failed</strong> • <?php echo (int) $ip['attempts']; ?> total
                    <br>Last: <?php echo date('M d, H:i', strtotime($ip['last_seen'])); ?>
                  </p>
                </div>
                <?php if (!empty($ip['is_banned']) && current_admin_role() === 'SuperAdmin'): ?>
                  <form method="post" style="margin-left: 12px;">
                    <input type="hidden" name="csrf_token" value="<?php echo sanitize($csrf_token); ?>">
                    <input type="hidden" name="unlock_ip_address" value="<?php echo sanitize($ip['ip_address']); ?>">
                    <button type="submit" class="btn btn-success btn-small" style="font-size: 11px; padding: 4px 8px;">
...
<script>
  function switchTab(tabId) {
    ...
    // Optional: save to sessionStorage to keep tab active after refresh
    sessionStorage.setItem('activeDashboardTab', tabId);
  }

  document.addEventListener('DOMContentLoaded', () => {
    // Check if there was a recent post (e.g. from security lock/unlock action)
    <?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['lock_admin_id']) || isset($_POST['unlock_ip_address']))): ?>
      switchTab('security');
    <?php else: ?>
