...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        if ($editing) {
            ...
            if ($stmt->execute()) redirect('news.php'); 
            else $error = 'Failed to save news.';
        } else {
            ...
            if ($stmt->execute()) redirect('news.php'); 
            else $error = 'Failed to create news.';
        }
    }
}
?>
    ...
    <div class="card-body">
      ...
      <form method="post" enctype="multipart/form-data">
        <div style="margin-bottom:8px">
          <label>Title</label>
          <input type="text" name="title" value="<?php echo sanitize($title); ?>" required style="width:100%;padding:8px;border:1px solid #e5e7eb;border-radius:8px">
        </div>
        <div style="margin-bottom:8px">
        ...
        <div style="margin-bottom:8px">
          <label>Image</label>
          <input type="file" name="image" accept="image/*" style="width:100%;padding:8px;border:1px solid #e5e7eb;border-radius:8px">
        </div>
        <div style="margin-bottom:8px">
          <label>YouTube URL</label>
          <input type="text" name="video_url" value="<?php echo sanitize($video_url); ?>" placeholder="https://www.youtube.com/watch?v=..." style="width:100%;padding:8px;border:1px solid #e5e7eb;border-radius:8px">
        </div>
        <div style="margin-bottom:12px">
          <label>Content</label>
          <textarea name="content" rows="8" style="width:100%;padding:8px;border:1px solid #e5e7eb;border-radius:8px"><?php echo sanitize($content); ?></textarea>
        </div>
        <div>
