...
try {
  ...
  if ($editing) {
    $stmt = $db->prepare("SELECT * FROM playoffs WHERE id=? LIMIT 1");
    $stmt->execute([$id]);
    $p = $stmt->fetch();
    if ($p) {
      $stage = $p['stage'];
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
      if ($editing) {
        $stmt = $db->prepare("UPDATE playoffs SET stage=?, start_date=?, end_date=?, home_team_id=?, away_team_id=?, home_score=?, away_score=?, winner_team_id=?, status=? WHERE id=? LIMIT 1");
        $stmt->execute([$stage, $start_date, $end_date, $home_team_id, $away_team_id, $home_score, $away_score, $winner_team_id, $status, $id]);
        audit_log($db, 'Edit Playoff', "Updated playoff bracket ID: $id ($stage)");
      } else {
        $stmt = $db->prepare("INSERT INTO playoffs(stage, start_date, end_date, home_team_id, away_team_id, home_score, away_score, winner_team_id, status) VALUES(?,?,?,?,?,?,?,?,?)");
        $stmt->execute([$stage, $start_date, $end_date, $home_team_id, $away_team_id, $home_score, $away_score, $winner_team_id, $status]);
        $newId = $db->lastInsertId();
        audit_log($db, 'Add Playoff', "Created playoff bracket ID: $newId ($stage)");
      }
      redirect('playoffs.php');
    }
  }
} catch (PDOException $e) {
  error_log("Playoff Form Error: " . $e->getMessage());
        ...
        <div class="grid col-2" style="margin-bottom:12px;">
          <div>
            ...
            <select name="home_team_id" required>
              ...
              while ($t = $teams->fetch_assoc()): ?>
                <option value="<?php echo (int) $t['id']; ?>" <?php echo ($home_team_id == (int) $t['id']) ? 'selected' : ''; ?>>
          ...
          <div>
            ...
            <select name="away_team_id" required>
              <?php $teams->data_seek(0);
              while ($t = $teams->fetch_assoc()): ?>
                <option value="<?php echo (int) $t['id']; ?>" <?php echo ($away_team_id == (int) $t['id']) ? 'selected' : ''; ?>>
                  <?php echo sanitize($t['name']); ?>
                </option>
              <?php endwhile; ?>
            </select>
